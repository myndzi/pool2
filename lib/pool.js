...
Pool.prototype._maybeAllocateResource = function () { // jshint maxstatements: 22, maxcomplexity: 8
    // do nothing if there are no requests to serve
    if (this.requests.length === 0) { return; }

    // call callback if there is a request and a resource to give it
    if (this.available.length) {
        var res = this.available.shift(), req;
        ...
        try {
            ...
            this._ping(res, function (err) {
                ...
                if (!req.fulfilled) {
                    ...
                    // abort() releases the request back to the request list -- we don't want to do that since it has been dealt with now
                }
            }.bind(this));
        } catch (err) {
