...
function validNum(opts, val, standard, allowZero) {
    if (!opts.hasOwnProperty(val)) {
        return standard;
    }
    var num = parseInt(opts[val], 10);
    if (isNaN(num) || num < 0) {
        throw new RangeError('Pool2: ' + val + ' must be a number, ' + opts[val] + ' given.');
    }
    if (!allowZero && num === 0) {
...
function Pool(opts) { // jshint ignore: line
    ...
    if (hasOwnProperty.call(opts, 'releaseTimeout')) {
    ...
    if (typeof opts.dispose !== 'function') {
        throw new Error('new Pool(): dispose function is required');
    }
    ...
    this.max = validNum(opts, 'max', 10);
    this.min = validNum(opts, 'min', 0, true);

    if (this.min > this.max) {
        throw new RangeError('Poo2: min cannot be greater than max');
    } 
    
    this.maxRequests = validNum(opts, 'maxRequests', Infinity);
    this.acquireTimeout = validNum(opts, 'acquireTimeout', 30 * 1000, true);
    this.disposeTimeout = validNum(opts, 'disposeTimeout', 30 * 1000, true);
    this.pingTimeout = validNum(opts, 'pingTimeout', 10 * 1000);
    this.idleTimeout = validNum(opts, 'idleTimeout', 60 * 1000);
    this.syncInterval = validNum(opts, 'syncInterval', 10 * 1000, true);

    if (this.syncInterval === 0 && this.idleTimeout) {
        throw new Error('Cannot specify idleTimeout when syncInterval is disabled');
    }
